...
Основные адреса:
- сайт: `http://localhost:26300`
- API docs: `http://localhost:26800/docs`
- internal playground: `http://localhost:26300/components-demo`
...
- реальные секреты хранятся только в `.env`, `.env.prod`, CI/CD secrets или host-level secret storage
- `NEXT_PUBLIC_*` не должен содержать секреты, потому что попадает в browser bundle
