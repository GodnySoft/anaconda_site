...
- `26300` — `web`
- `26800` — `api`
- `26543` — `postgres` из dev docker-контура
- `26081` — `pgweb` debug profile

Проверка:

```bash
ss -ltnp '( sport = :26300 or sport = :26800 or sport = :26543 or sport = :26081 )'
```

Если на этих портах висит старый процесс, его нужно остановить до запуска docker-контура.
...
curl -f http://127.0.0.1:26800/docs
...
```bash
curl -f -X POST http://127.0.0.1:26800/api/v1/leads \
...
Важно: перед `prod-smoke` dev-контур на `26300/26800` должен быть остановлен, иначе будет конфликт портов.
...
```bash
ss -ltnp '( sport = :26300 or sport = :26800 )'
```
